...
add_action(
	'admin_notices',
	function () {
		$wpforms_pro_installed = false;
		if ( function_exists( 'wpforms' ) && wpforms()->is_pro() ) {
			$wpforms_pro_installed = true;
		}
		if ( ! $wpforms_pro_installed ) {
			if ( current_user_can( 'update_plugins' ) ) {
				?>
				<div class="admin-notice notice notice-warning">
					<p>
						<strong>WPForms Views</strong> depends on WPForms Pro. Please install <a target="_blank" href="https://wpforms.com/pricing/">WPForms Pro</a>.
					</p>
				</div>
				<?php
			}
...
function wpforms_views_frontend_scripts() {
	wp_enqueue_style( 'pure-css', WPFORMS_VIEWS_URL_LITE . '/assets/css/pure-min.css' );
	wp_enqueue_style( 'pure-grid-css', WPFORMS_VIEWS_URL_LITE . '/assets/css/grids-responsive-min.css' );
	wp_enqueue_style( 'wpforms-views-front', WPFORMS_VIEWS_URL_LITE . '/assets/css/wpforms-views-display.css', array(), time() );
}
